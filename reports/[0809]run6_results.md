...
5. 색 지표(`dE_unbraid`, `lpips_unbraid`)는 run5_1 대비 두 run
   모두 epoch15에서 소폭 더 나쁨(§3.3) 
...
## 4. 한계
1. **조건당 학습 반복 없음(`n=1`)** — 학습 시드는 프로젝트 관행상 원래 고정하지 않음
   (`trainer.py:193`, run4~run5_x와 동일한 관행). 문제는 미고정 자체가 아니라, 같은
   `w_lpips` 값으로 학습을 반복하지 않아 "그 값의 진짜 효과"와 "그 1회 학습이 우연히 뽑은
   초기화·데이터 순서·noise 샘플링"을 분리할 수 없다는 점임. §3.2의 비단조 패턴
   (`run5_1 < run6_2 < run6_1`)이 8/8 이미지에서 일관되긴 하나, 조건당 반복 학습 없이는
   이게 `w`의 진짜 효과인지 그 1회 학습들이 우연히 뽑은 결과인지 이 리포트만으로는
   확정할 수 없음.
