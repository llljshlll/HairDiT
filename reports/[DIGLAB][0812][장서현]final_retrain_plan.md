...
확인 결과(diffusers 공식 코드): SD3.5는 텍스트 프롬프트만 dropout함(`train_controlnet_sd3.py --proportion_empty_prompts`, 기본값 0). SD3 ControlNet 자체 예제 명령어는 이 값을 아예 안 정해서(=0, dropout 없음) 씀 — SDXL/Flux-control 등 이웃 모델 예제에서만 0.2를 씀. 어느 쪽이든 텍스트 dropout 비율이지 이미지/residual 조건 dropout이 아님 이미지 조건 dropout의 실측 선례는 diffusers 전체에서 **InstructPix2Pix**(이미지+텍스트 지시문으로 이미지를 편집하는 SD 기반 모델, 예: "이 사진을 수채화로 바꿔줘". 조건이 텍스트·이미지 둘이라 각각 독립적으로 dropout해 학습함, 0.05)가 유일해서 이 값들을 그대로 못 옮겨 씀.
...
- **안 B(sketch만 dropout, matte는 유지)**: 걸린 샘플은 ControlNet 입력 중 sketch만 0(matte는 그대로) → `v_cond−v_uncond`가 방향·색 정보만 담고 matte 위치정보는 안 섞임 — 이론적으로 더 정합적일 것이라는 추론. uncond 분기도 ControlNet을 돌려야 해 추론 비용이 현재(~1.5배)에서 ~2배로 늘고, 현재 되어있는 CRG 구현도 손봐야 함. - 만약 진행한다면, 일단 지금 이미 학습되어있는 run5_1에서 해당 방식으로 추론 해보고 결정
### dropout없이 진행했을 때 A안과 B안 비교

방법: run5_1(`checkpoints/run5_1_noisegate/epoch_15_infer.pth`, epoch15) × CRG 2.0, `data/test` 8장, seed 42, 20-step, `bld_mode full`(soft_steps 18) + pixel_blend(alpha 0.75) 공통. 안 A/안 B 모두 학습 시 dropout은 없음(현재 체크포인트 그대로) — uncond 분기 구성만 추론 시점에서 다르게 흉내낸 것(`scripts/infer_custom.py --cfg_uncond_mode {residual_off, sketch_zero}`).
...
run4(40epoch) × colorful sketch(원본 색 스케치, GT 재채색 없음) 비교. 방법은 위와 동일(CRG 2.0, seed 42, 20-step, BLD full/soft_steps 18 + pixel_blend 0.75), 체크포인트만 `checkpoints/run4_phase1/epoch_40_infer.pth`(config `configs/lpips_low_phase1.yaml`), 입력 스케치만 `data/test/sketch`(GT 재채색 없는 원본 색).
