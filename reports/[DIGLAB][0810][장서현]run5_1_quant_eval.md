...
- 결과: 50장 정량 평가에서 **방향 지표는 run5_1이 최선**(GT 오차 14.95°, seed 불일치
 13.14°)이나, **색/구조 지표는 run6_2가 최선**(dE_unbraid 4.16, lpips_unbraid 0.2087) —
...
- 추론: `scripts/infer_custom.py --recolor_from_gt`(GT 색으로 stroke 재채색, §0 `run5_1`
 ...
 epoch15 고정, seed `{1,2,3,42}`, 20 step. 로컬 RTX 4070(12GB)에서 12개 조합(3 run × 4 seed) 순차 실행,
...
|---|---:|---:|---:|
| **run5_1** | **14.95** | 0.769 | **13.14±4.64** |
| run6_1 | 16.16 | 0.739 | 15.10±4.86 |
| run6_2 | 15.26 | **0.775** | 13.68±5.01 |
...
모두 run5_1이 최선, run6_1이 최악. coherence만 run6_2(0.775)가 run5_1(0.769)을 근소하게
앞섬(8장 평가에서는 run5_1 0.770 > run6_2 0.758로 반대 순서였음 — 차이가 작아 이
역전을 실질적 신호로 보지는 않음).
...
|---|---:|---:|
| run5_1 | 4.5783 | 0.2222 |
| run6_1 | 4.2429 | 0.2195 |
| **run6_2** | **4.1628** | **0.2087** |
...
| 지표 | 최선 | 최악 |
|---|---|---|
| GT 방향 오차 | run5_1 (14.95°) | run6_1 (16.16°) |
| seed 불일치 | run5_1 (13.14°) | run6_1 (15.10°) |
| dE_unbraid | run6_2 (4.16) | run5_1 (4.58) |
| lpips_unbraid | run6_2 (0.2087) | run5_1 (0.2222) |
